...
namespace Quantum.Menu {
  ...
  public partial class QuantumMenuConnectArgs {
    ...
    public QuantumMenuSceneInfo Scene {
      ...
      set {
        _scene = value;
        SceneName = value.NameOrSceneName;
      }
    }

    /// <summary>
    /// Selected scene saved on player prefs.
    /// </summary>
    [HideInInspector]
    ...
    private QuantumMenuSceneInfo _scene;
    ...
    public void LoadFromPlayerPrefs(string keyPrefix = "Photon.Menu.") {
      ...
      SceneName = PlayerPrefs.GetString($"{keyPrefix}SceneName");
    ...
    public void SaveToPlayerPrefs(string keyPrefix = "Photon.Menu.") {
      ...
      PlayerPrefs.SetString($"{keyPrefix}SceneName", SceneName);
    ...
    public virtual void SetDefaults(QuantumMenuConfig config) {
      ...
      if (config.AvailableSceneAssets.Count > 0) {
        var index = config.AvailableSceneAssets.FindIndex(s => string.IsNullOrEmpty(SceneName) == false && s.NameOrSceneName == SceneName);
        if (index >= 0) {
          // Overwrite anything in storage with fresh information from the config
          Scene = config.AvailableSceneAssets[Mathf.Clamp(index, 0, config.AvailableSceneAssets.Count - 1)];
        } else {
          // Always set a valid scene if any are available.
          Scene = config.AvailableSceneAssets[0];
        }
      }
...
namespace Quantum.Menu {
  ...
  public partial class QuantumMenuMppmJoinCommand : QuantumMppmCommand {
    ...
    private async Task ExecuteAsync() {
      ...
      // Skip when this client is already connecting
